...
use AppBundle\Entity\AgreementVehicle;

/**
 * Class LoadAgreementVehicleData
 */
class LoadAgreementVehicleData extends AbstractFixture implements OrderedFixtureInterface, ContainerAwareInterface
{
    ...
    public function load(ObjectManager $manager)
    {
        ...
        for ($i = 1; $i <= $willBeAdded; $i++) {
            $newAgreement = new AgreementVehicle();
            
            $newAgreement->setNumber(str_pad($i + 11, 5, 0, STR_PAD_LEFT));
            $newAgreement->setVehicle($faker->sentence(2));
            $newAgreement->setValue($faker->randomFloat(300, 800));
